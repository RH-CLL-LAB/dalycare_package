CTCAE_lab = function(data, patientid = patientid, NPU = NPU, value = value2, 
                     ref_lower = referenceinterval_lowerlimit, ref_upper = referenceinterval_upperlimit, samplingdate = samplingdate) {
  #' @title
  #' CTCAE_lab
  #' @author
  #' christian brieghel
  #' @description
  #' This function transforms laboratory values from SDS_lab_forsker (LABKA), 
  #' SP_AlleProvesvar, and/or laboratorymeasurements (aggregated) to CTCAEv5.0 defined adverse events
  #' author: christian brieghel
  #' @note 
  #' Defines AEs: 
  #' 1. Dissiminated intravascular coagulation (DIC), DIC.GRADE
  #' 2. Hemolysis, HEM.GRADE
  #' Expect time-lapse for large samples; consider down sampling
  #' 
  #' @examples 
  #' load_npu_common()
  #' HGB = load_biochemistry(NPU.HGB) %>% clean_lab_values()
  #' ANEMIA_AE = HGB %>% CTCAE_lab() %>% select(patientid, ANEMIA.GRADE, everything())
  #' HEMOLYSIS = load_biochemistry(NPU.GROUP.HEMOLYSIS) %>% clean_lab_values() 
  #' HEMOLYSIS_AE = HEMOLYSIS %>% CTCAE_lab() %>% select(patientid, HEM.GRADE, everything()) 
